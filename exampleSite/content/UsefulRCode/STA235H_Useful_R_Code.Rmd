...
For this class, make sure you have the most updated version of R (you can check your R version by typing `version` on the console). If your version is older than `R 4.2.1`, please [download](https://cran.r-project.org/bin/windows/base/) and install the latest R version.
...
<br>
```{r clean_space, fig.align = 'center', out.width = "80%", echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/UsefulRCode/images/global_options.png")
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE, align = 'center'}
...
- `pch`: In a scatter plot, defines the type of marker you are using (e.g. circles, squares, triangles). Check out the different options [here](http://www.sthda.com/english/wiki/r-plot-pch-symbols-the-different-point-shapes-available-in-r)
...
<br>
```{r convincing, fig.align = 'center', out.width = "80%", echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/UsefulRCode/images/convincing.png")
```
...
```{r join, echo = FALSE,fig.align = 'center', out.width = "50%", fig.cap = "Overview of the dplyr Join Functions ([Source](Source: https://statisticsglobe.com/r-dplyr-join-inner-left-right-full-semi-anti))"}
...
```{r, echo = TRUE, warning = FALSE, message = FALSE, results='asis', align = 'center'}
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, results="asis", align = 'center'}
lm0 <- lm(rating ~ sugars, data = d)
lm1 <- lm(rating ~ sugars + fat, data = d)
lm2 <- lm(rating ~ sugars + fat + fiber + cups, data = d)

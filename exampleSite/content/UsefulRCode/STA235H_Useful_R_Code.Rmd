...
Besides what we will see in class, you can use [`flextable`](https://davidgohel.github.io/flextable/) to create a table of your dataset (or a subset of the most relevant variables). For example, you can use `continuous_summary()`:
...
flextable::continuous_summary(cereal, digits = 2) %>% 
  set_caption(., "Summary of my cereal dataset")
```

You can also export your table in different formats (e.g. docx, html, tex, ppt). Just add the respective function after, including your file name (and path):

```{r, echo = TRUE, warning = FALSE, message = FALSE, eval = FALSE}
continuous_summary(d, digits = 2) %>% 
  set_caption(., "Summary of my cereal dataset") %>%
  save_as_docx(path = "cereal_summary.docx")
```

For a regression, you can a do a similar thing:
...
Make sure to check out the additional arguments that `stargazer` can take! You can really customize your tables with this package.

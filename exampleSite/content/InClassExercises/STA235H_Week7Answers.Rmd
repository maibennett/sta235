...
3) (If there is time) Compare the characteristics between your treatment and your control group (e.g. income, gender, employment). Do they look balanced?

```{r, message = FALSE, warning=FALSE}
netflix_bal = netflix %>% select(treat, income, employed, female)

datasummary_balance(~ treat, data = netflix_bal, dinm_statistic = "p.value",
                    fmt = 3)
```

4) Calculate 4 averages for the outcome of interest: 
...
```{r, message = FALSE, warning=FALSE}
netflix %>% group_by(treat, post) %>% summarise(avg_subs = round(mean(subscribed), 3))
```
...
- What is the difference in subscriptions between the treatment and the control group in a) April 2022, and b) July 2022?

In April: $0.635 - 0.515 = 0.12$

In July: $0.543 - 0.488 = 0.055$
...
On average, the policy increased subscriptions in NY and CA by `r round(lm_dd$coefficients['treat:post'],3)*100` percentage points compared to no policy.

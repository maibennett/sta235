...
```{r jitt_dt, fig.height=5.5, fig.width=7.5, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message = FALSE}
library(ggplot2)
library(gridExtra)
library(hrbrthemes)
library(firasans)

jitt <- read.csv("https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/Week12/1_DecisionTrees/data/jitt.csv")

jitt2 <- as.data.frame(cbind(as.vector(table(jitt$Answer)),rownames(table(jitt$Answer))))
names(jitt2) = c("value","bin")
jitt2$bin <- factor(jitt2$bin,levels = c("l","m","o"), labels = c("Logged in past week", "Mandalorian", "Other"))
...
<br>
.box-7[3) Stop if leaves are pure or early stopping criteria is satisfied, else repeat step (1) and (2) for each new child nodes]
<br>
.box-7[4) Prune your tree according to a complexity parameter (cp). Choose the optimal one through CV.]

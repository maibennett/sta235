...
- Review session for the midterm on **.darkorange[Thursday 6.00pm]**
...
- **.darkorange[Natural Experiments]**
  ...
  - Importance of the context and checking potential confounding.
...
- **.darkorange[Structure for today]**:
...
--
<br>
.box-4t[... but there are robustness checks (e.g. check covariates at the threshold)]
...
$$Y_i = \beta_0 + \beta_1R' + \beta_2Z + \beta_3R'\times Z$$

where $Z$ is a binary treatment variable and $R'$ is the running variable centered around the cutoff
...
1) Can you compare sales for the first 1,000 customers with sales for 1,001-2,000 to get a causal effect?

--

2) Can you compare sales for the first 1,000 customers today vs 1,000 customers yesterday to get a causal effect?

--

3) What could you do to get a causal effect?
...
$$Y = \beta_0 + \beta_1f(R') + \beta_2Z + \beta_3f(R')\times Z + \varepsilon$$
...
- Go to **.darkorange[[sta235h.clicks/Week8](https://sta235h.clicks/Week8)]**
...
For conducting an RDD:

1) Check that there is a **.darkorange[discontinuity in assignment]** (e.g. plot `treat` vs `r`)

2) Check that **.darkorange[covariates are smooth across the cutoff]** (e.g. RDD for `x` (baseline covariate))

3) Run a **.darkorange[RDD]** using either a linear model (`lm()`) or a nonparametric model (`rdrobust()`)

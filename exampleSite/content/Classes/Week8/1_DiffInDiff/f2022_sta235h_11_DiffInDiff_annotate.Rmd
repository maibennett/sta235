---
title: "STA 235H - Natural Experiments & Difference-In-Differences"
subtitle: "Fall 2022"
...
.center[What do we do if we have something like a natural experiment but **.darkorange[both our groups are not necessarily balanced]**?]
...
minwage <- read.csv("https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/Week8/1_DiffInDiff/data/min_wage.csv")

minwage <- minwage %>% mutate(treat = ifelse(grp=="PA", 0, 1), # treat group: the treated state
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
summary(lm(fte ~ treat*post, data = minwage))
...
*"Increasing the minimum wage has an average effect <u>in New Jersey</u> of 2.9 additional jobs per fast food restaurant (though the effect is not statistically significant at conventional levels)"*
...
- Each group will have a different example

- Go to **.darkorange[stah235.rocks/groupX]** (e.g. group1, group2, ...)

- Read the brief and answer the questions.

- Write down your answers and **.darkorange[be ready to present to the class]**.

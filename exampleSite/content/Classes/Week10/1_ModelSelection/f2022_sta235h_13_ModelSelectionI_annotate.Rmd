...
xb_l <- 2.5 - 5*mandalorian + city*2 + rnorm(n, 0, 1)
...
xb <- 0.15 - 0.01*city + mandalorian*0.2 + (logins==0)*0.1 + rnorm(n, 0, 0.05)
...
train.data <- disney %>% slice(train)
test.data <- disney %>% slice(-train)
```
...
lm_simple <- lm(logins ~ mandalorian + city, data = train.data)

lm_complex <- lm(logins ~ female + city + age + I(age^2) + mandalorian, data = train.data)

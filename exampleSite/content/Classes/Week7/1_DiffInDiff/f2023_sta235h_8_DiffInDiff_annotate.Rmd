...
- Difference-in-Differences provides an estimate for an **.darkorange[average treatment effect for the treated group]**
  ...
  - The estimated effect is not generalizable for the entire sample, *only for the treated gorup*.
...
```{r plot-trends-1, fig.height=5.25, fig.width=12, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message = FALSE}
dd_labels <- tribble(
  ~Group, ~Time, ~label,
  "Treatment", "Before", "A",
  "Treatment", "After", "B",
...
```{r plot-trends-2, fig.height=5.25, fig.width=12, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message = FALSE}
dd_labels <- tribble(
  ~Group, ~Time, ~label,
  "Treatment", "Before", "A",
  "Treatment", "After", "B",
...
ggplot(dd_data, aes(x = Time, y = avg, color = Group, group = Group)) +
  ...
  geom_line(size = 3,
            key_glyph = draw_key_point) +
  ggplot2::annotate(geom = "segment", x = "Intervention", xend = "After",
           y = 32, yend = 34, color = "#900DA4", size = 2, linetype = "22") +
  ggplot2::annotate(geom = "segment", x = "After", xend = "After",
           y = 34, yend = 50, color = "#FF4136", size = 2) +
  ggplot2::annotate(geom = "label", x = "After", y = 44, label = "Causal\neffect",
           family = "Fira Sans Condensed", fontface = "bold", 
           size = 4, fill = "#FF4136", color = "white") +
  geom_point(data = filter(dd_data, !is.na(label)), size = 15) +
...
```{r plot-trends-3, fig.height=5.25, fig.width=12, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message = FALSE}
dd_labels <- tribble(
  ~Group, ~Time, ~label,
  "Treatment", "Before", "A",
  "Treatment", "After", "B",
...
ggplot(dd_data, aes(x = Time, y = avg, color = Group, group = Group)) +
  ...
  geom_line(size = 3,
            key_glyph = draw_key_point) +
  ggplot2::annotate(geom = "segment", x = "Intervention", xend = "After",
           y = 32, yend = 37, color = "#F89441", size = 2, linetype = "22") +
  ggplot2::annotate(geom = "segment", x = "Intervention", xend = "After",
           y = 32, yend = 34, color = "#900DA4", size = 1, linetype = "22") +
  ggplot2::annotate(geom = "segment", x = "After", xend = "After",
           y = 37, yend = 50, color = "#FCCE25", size = 2) +
  ggplot2::annotate(geom = "label", x = "After", y = 44, label = "DD\nestimate",
           family = "Fira Sans Condensed", fontface = "bold", 
           size = 4, fill = "#FCCE25", color = "white") +
  geom_point(data = filter(dd_data, !is.na(label)), size = 15) +
  geom_text(aes(label = label), family = "Fira Sans Condensed", 
            fontface = "bold", size = 9, color = "white") +
  scale_color_manual(values = c("#900DA4", "#F89441")) +
...
```{r plot-pretrends-1, fig.height=5.25, fig.width=12, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message = FALSE}
dd_labels <- tribble(
  ~Group, ~Time, ~label,
  "Treatment", "Before", "A",
  "Treatment", "After", "B",
...
dd_data <- tribble(
  ~Group, ~Before, ~`Pre-intervention`, ~Intervention, ~After,
  "Treatment", 30, 32.5 , 35, 50,
  "Control",   20, 22.5 , 25, 30
) %>% 
  pivot_longer(cols = !Group, names_to = "Time", values_to = "avg") %>% 
  left_join(dd_labels, by = c("Group", "Time")) %>% 
  mutate(across(c(Group, Time), ~fct_inorder(.)))
ggplot(dd_data, aes(x = Time, y = avg, color = Group, group = Group)) +
  geom_vline(xintercept = "Pre-intervention") +
  geom_line(size = 3,
            key_glyph = draw_key_point) +
  ggplot2::annotate(geom = "segment", x = "Intervention", xend = "After",
           y = 35, yend = 40, color = "#900DA4", size = 2, linetype = "22") +
...
- Go to **.darkorange[[sta235h.click/Week7](https://sta235h.click/Week7)]**

...
- Feedback for the **.darkorange[project's idea pitch]** was posted!

  - Review the comments and incorporate them moving forward (e.g. switch from causal inference to prediction).
  - If you have questions, <u>reach out</u>!
  - Next submission is on Nov 10th (check the website for details)
  
--

- Remember that you can **.darkorange[drop an assignment]**:

  - That is meant to be used in any situation you can't submit on time.
  
--

- In Canvas, all open attempts are submitted by the assignment deadline **.darkorange[for closure.]**

  - If you stop working on your assignment before the submission deadline, penalty doesn't count.
  
--

- Office hours on Thursday will start at **.darkorange[4.00pm]**
...
.center[
![:scale 55%](https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/Week9/1_RDD/images/voting_share.png)]
...
- Threshold rule **.darkorange[exists]** and cutoff point is **.darkorange[known]**
  ...
  - How do we check that? Check that **.darkorange[there is a discontinuity in treatment assignment at the cutoff]**

--

- The running variable $R_i$ is **.darkorange[continuous]** near $c$.

  - How do we check that? Check that **.darkorange[the density of the running variable is smooth across the cutoff]**
...
$$Y = \beta_0 + \beta_1\color{#900DA4}{\underbrace{(R-c)}_\text{Distance to the cutoff}} + \beta_2\mathrm{I}[R>c] + \beta_3\color{#900DA4}{\overbrace{(R-c)}^\text{Distance to the cutoff}}\mathrm{I}[R> c] + \varepsilon$$
--
...
1) Can you compare sales for the first 1,000 customers with sales for 1,001-2,001 to get a causal effect?
...
sales <- read.csv("https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/Week9/1_RDD/data/sales.csv")
...
.center[
![:scale 20%](https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/Week9/1_RDD/images/confusion.svg)
]
--
...
- `rddensity`: Check density across the cutoff (from the `rddensity` package)
...
- Go to **.darkorange[[sta235h.rocks/Week9](https://sta235h.rocks/Week9)]**

...
- Some ways to **.darkorange[select models]**:
  ...
  - Shrinkage methods: Ridge and Lasso.
    ...
    - When is ridge regression better? When is lasso better?
]
...
hbo <- read.csv("https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/Week11/1_ModelSelection/data/hbomax.csv")
...
- In an **.darkorange[OLS]**: Minimize sum of squared-errors, i.e. $\min_\beta \sum_{i=1}^n(\text{spend}_i - \beta\text{freq}_i)^2$
...
.center2[
.box-2LA[Why does Ridge Regression reduce its slope compared to OLS?]
]
...
$$\min_\beta \sum_{i=1}^n\underbrace{(y_i - \beta_0 - x_i\beta)^2}_{OLS} + \color{#F89441}{\underbrace{\lambda\cdot \beta_1^2}_{Ridge Penalty}}$$
--
...
#  geom_segment(data = test, aes(xend = freq, yend = fitted), lwd = 1.5, color = "#FCCE25") +
  ...
  geom_point(data = test, aes(fill = "#5601A4"), pch = 21, color = "#5601A4", size = 5) +
  scale_fill_manual(values=c("#5601A4","#BF3984"),labels = c("#BF3984" = "Training","#5601A4" = "Testing"), name = "Data") +
  annotate("text",x=0.5, y = 150, color = "dark grey", label = "y = 132.5 - 16.25xfreq", size = 5, hjust = 0)+
  annotate("text",x=7, y = 60, color = "#F89441", label = "y = 119.5 - 11.25xfreq", size = 5, hjust = 0)+
  theme_bw()+
  theme_ipsum_fsc() + #plain 
  xlab("Frequency") + ylab("Spending ($)")+
...
- **.darkorange[Yes!]** This is a stylized example to show what's happening in the background when we are running OLS and Ridge regression.

--

- How can we know whether **.darkorange[OLS]** or **.darkorange[Ridge Regression]** is better without running the risk of cherry-picking training and testing data?

--

- If the data is linear, **.darkorange[OLS]** might be the right model:

  - **.darkorange[Penalty term &lambda;]** will most likely be 0.

---
.center[
![:scale 90%](https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/week11/1_Shrinkage/images/ridge.png)]
...
.center2[
.box-5LA[Let's look at this in R!]
]
...
cvl_lambda <- data.frame(lambda = lasso$results$lambda,
...
<!-- pagedown::chrome_print('C:/Users/mc72574/Dropbox/Hugo/Sites/sta235/exampleSite/content/Classes/week11/1_Shrinkage/f2021_sta235h_15_Shrinkage.html') -->
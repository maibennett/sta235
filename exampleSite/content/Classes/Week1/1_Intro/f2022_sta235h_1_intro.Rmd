...
```{r syllabus, fig.height=6, fig.align='center', dev='svg', echo=FALSE, warning=FALSE}
library(tidyverse)
library(hrbrthemes)
library(firasans)
library(patchwork)
...
g_ext <- d %>% filter(!is.na(points_ext)) %>% rename(ext = X28697445..True.or.False...It.s.ok.if.I.submit.a.late.assignment.because.there.are.extensions.) %>%
  ...
  ggtitle("It's ok to submit late\nassignments because there are extensions") + 
  theme_void()+
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 14) + 
  theme(plot.margin=unit(c(0.5,0.5,0.5,0.5),"cm"),
        panel.grid.major.x = element_blank(),
...
g_off + g_att + g_jitt + g_hw + g_ext + g_ppt + plot_layout(ncol = 3)
...
- CAVEAT ABOUT TIME TO DO THE ASSIGNMENTS.
...
- **.darkorange[Midterm (15%) and Final Exam (20%):]**
  ...
  - Take-home exam (timed). Final exam is cumulative.
...
- **.darkorange[Participation (5%):]**
  ...
  - Attendance will be taken on 5 random classes. You can be absent in one of them without justification.
...
- There are **.darkorange[no extensions]** for assignments, but you can drop **.darkorange[drop one (1) HW]** and **.darkorange[drop one (1) JITT]**.

  - Assignments are posted with enough time to account for frequent issues (e.g. sickness, interviews, etc.)
...
.source[There can be curving in individual assignments, but NOT for the final grade.<br>There will not be extra credit]

...
--
.pull-right[
.box-4transl[T.A.: Shentao Yang (Ph.D. student)]


.box-6transl[T.A.: Katie Aufricht (3rd-year Honors)]


.box-7transl[T.A.: Isabella Hsu (3rd-year Honors)]
]
...
... I have a very cute pup.


.center[
![:scale 40%](https://github.com/maibennett/sta235/raw/main/exampleSite/content/Classes/Week1/1_Intro/images/BobDylan.png)
]
---
background-position: 50% 50%
class: center, middle
...
d <- read.csv("https://raw.githubusercontent.com/maibennett/sta235/main/exampleSite/content/Classes/Week1/1_Intro/data/intro_survey.csv")

cols = c("#BF3984","#E16462","#F89441","#FCCE25")

g_off <- d %>% filter(!is.na(points_off)) %>% rename(office = X28697441..Where.is.Professor.Bennett.s.office.) %>%
  mutate(office = as.factor(office)) %>%
  group_by(office) %>% summarize(value = n()) %>%
  ggplot(data = ., aes(x="", y = value, fill = factor(office))) +
  geom_bar(lwd=1.2, stat="identity", color="white") +
  coord_polar("y",start = 0)+
  scale_fill_manual(values=c(cols[1], cols[3])) + 
  ggtitle("Where is Prof. Bennett's office?") + 
  theme_void()+
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 14) + 
...
g_att <- d %>% filter(!is.na(points_att)) %>% rename(attendance = X28697443..Does.attendance.count.towards.your.grade.) %>%
  mutate(attendance = as.factor(attendance)) %>%
  group_by(attendance) %>% summarize(value = n()) %>%
  ggplot(data = ., aes(x="", y = value, fill = factor(attendance))) +
  geom_bar(lwd=1.2, stat="identity", color="white") +
  coord_polar("y",start = 0)+
  scale_fill_manual(values=c(cols[3],cols[1])) + 
  ggtitle("Does attendance count\ntowards your grade?") + 
  theme_void()+
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 14) + 
  theme(plot.margin=unit(c(0,0,0,0),"cm"),
        panel.grid.major.x = element_blank(),
...
g_jitt <- d %>% filter(!is.na(points_jitt)) %>% rename(jitt = X28697446..True.or.False...JITTs.are.due.every.week.) %>%
  mutate(jitt = as.factor(jitt)) %>%
  group_by(jitt) %>% summarize(value = n()) %>%
  ggplot(data = ., aes(x="", y = value, fill = factor(jitt))) +
  geom_bar(lwd=1.2, stat="identity", color="white") +
  coord_polar("y",start = 0)+
  scale_fill_manual(values=c(cols[3],cols[1])) + 
  ggtitle("JITTs are due every\nweek") + 
  theme_void()+
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 14) + 
  theme(plot.margin=unit(c(0,0,0,0),"cm"),
        panel.grid.major.x = element_blank(),
...
g_hw <- d %>% filter(!is.na(points_hw)) %>% rename(hw = X28697442..How.many.homework.assignments.can.you.drop.) %>%
  mutate(hw = as.factor(hw)) %>%
  group_by(hw) %>% summarize(value = n()) %>%
  ggplot(data = ., aes(x="", y = value, fill = factor(hw))) +
  geom_bar(lwd=1.2, stat="identity", color="white") +
  coord_polar("y",start = 0)+
  scale_fill_manual(values=cols[1]) + 
  ggtitle("How many homework assignments\ncan you drop?") + 
  theme_void()+
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 14) + 
  theme(plot.margin=unit(c(0,0,0,0),"cm"),
        panel.grid.major.x = element_blank(),
...
g_ext <- d %>% filter(!is.na(points_ext)) %>% rename(ext = X28697445..True.or.False...It.s.ok.if.I.submit.a.late.assignment.because.there.are.extensions.) %>%
  mutate(ext = as.factor(ext)) %>%
  group_by(ext) %>% summarize(value = n()) %>%
  ggplot(data = ., aes(x="", y = value, fill = factor(ext))) +
  geom_bar(lwd=1.2, stat="identity", color="white") +
  coord_polar("y",start = 0)+
  scale_fill_manual(values=c(cols[1],cols[3])) + 
  ggtitle("It's ok to submit late\nassignments because there\nare extensions") + 
  theme_void()+
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 14) + 
  theme(plot.margin=unit(c(0,0,0,0),"cm"),
        panel.grid.major.x = element_blank(),
...
g_ppt <- d %>% filter(!is.na(points_ppt)) %>% rename(ppt = X28697444..Do.you.have.to.be.present.for.the.final.project.presentations.) %>%
  mutate(ppt = as.factor(ppt)) %>%
  group_by(ppt) %>% summarize(value = n()) %>%
  ggplot(data = ., aes(x="", y = value, fill = factor(ppt))) +
  geom_bar(lwd=1.2, stat="identity", color="white") +
  coord_polar("y",start = 0)+
  scale_fill_manual(values=c(cols[1])) + 
  ggtitle("Do you have to be present\nfor the final project presentations?") + 
  theme_void()+
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 14) + 
  theme(plot.margin=unit(c(0,0,0,0),"cm"),
        panel.grid.major.x = element_blank(),
...
```{r syllabus, fig.height=4.5, fig.width=10, fig.align='center', dev='svg', echo=FALSE, warning=FALSE}
g_off + g_att + g_jitt + g_hw + g_ext + g_ppt + plot_layout(nrow = 2)
#plot_grid(g_off,g_att,g_jitt,g_hw,g_ext,g_ppt, nrow = 2, align = 'hv')
...
- **.darkorange[In-person (Fall 2022)]:** 2 hrs/week at UTC 1.102
...
- All assignments and exams are **.darkorange[take-home]** and **.darkorange[open-book]**.

--

- Provide **.darkorange[structured flexibility]** in submission of assignments to avoid academic inequality:

  - E.g. HW assignments $\sim$ 2-3hrs $\rightarrow$ 1 week: Accounts for (typical) foreseeable and unforeseeable events
...
- In the case of other circumstances that do not fit in the previous description, **.darkorange[please reach out]**.
...
- **.darkorange[Just in Time Teaching (JITT) assignments (10%):]**
  ...
  - Short online questionnaires about readings and/or material.
...
- **.darkorange[7 homework assignments (30%):]**

  - All in Canvas.
  - Assignments include both written questions and code.
  - You can drop one hw assignment (only 6 will count).
  - Assignments are individual. No collaboration, copying, or plagiarism will be accepted.
...
- **.darkorange[Midterm (15%) and Final Exam (20%):]**

  - Take-home exam (restricted time). Final exam is cumulative.
  
- **.darkorange[Final project (20%):]**

  - Group project about data analysis.
  
- **.darkorange[Participation (5%):]**

  - Attendance will be taken on 5 random classes. You can be absent in one of them without penalty.
  
  - If you miss more than one (1) of those classes, you can make up with participation. 
...
- General questions should be posted on Canvas (Chatter)
...
- You can post with your name or anonymously (see instructions on Canvas)
]  

.pull-right[
.center[
![:scale 100%](https://github.com/maibennett/sta235/raw/main/exampleSite/content/Classes/Week1/1_Intro/images/chatter.png)]]
...
- **.darkorange[You are encouraged to form study groups!]**
  ...
  - Studying or discussing class material with others does **.darkorange[not]** mean you can copy or directly collaborate on assignments.
...
d_text <- data.frame(word = names(v),freq=v, stringsAsFactors = FALSE)
#head(d, 10)

d_text$word[d_text$word=="r"] = "R"
...
wordcloud(words = d_text$word, freq = d_text$freq, min.freq = 1,
          max.words=200, random.order=FALSE, rot.per=0.35, 
...
d_aux <- d %>% filter(!is.na(X28697439.....How.comfortable.are.you.with.coding.in.R...Select.....))

d_aux <- d_aux %>% rename(comfortable_r = X28697439.....How.comfortable.are.you.with.coding.in.R...Select.....) %>% 
  mutate(comfortable_r = ifelse(comfortable_r == "1 (Really unfcomfortable)", 1, comfortable_r),
         comfortable_r = ifelse(comfortable_r == "10 (Super comfortable)", 10, comfortable_r),)

ggplot(data = d_aux, aes(x = factor(comfortable_r,
                                levels = seq(1,10,1)))) + 
...
d_aux <- d %>% rename(grade_expected = X28697440.....Thinking.towards.the.end.of.the.semester..what.grade.do.you.think.you.will.get.in.this.course...Select.....) %>%
  filter(grade_expected!="")

d_aux$grades = factor(d_aux$grade_expected,
                  levels = c("A","A-","B+","B","B-","C+","C or less"))

d2 = as.data.frame(cbind(table(d_aux$grades),rownames(table(d_aux$grades))), stringsAsFactors = FALSE)
names(d2) = c("value","Grades")

d2$Grades = factor(d2$Grades, levels = c("A","A-","B+","B","B-","C+","C or less"))

cols = c("#0D0887","#900DA4","#BF3984","#900DA4","#F89441","#FCCE25","#f5dc7f")
...
- We will be using a **.darkorange[seating chart]** for contact-tracing purposes:

  - Choose your preferred seat during the break and then write it down on the sitting chart I'll pass around.

...
round(summary(lm(log(Adj_Revenue)~factor(pass_bechdel)+log(Adj_Budget) + Metascore + imdb, 
           data=bechdel))$coeff[1:5,], 4)
...
![:scale 100%](https://github.com/maibennett/sta235/raw/main/exampleSite/content/Classes/Week1/2_OLS/images/linear_regression.png)
---
# Some measures to quantify uncertainty

- .darkorange[$\mathbb{R^2}$:] Correlation between the outcome $\mathbb{Y}$ and the predicted variable $\mathbb{\hat{Y}}$.
...
res_data = as.data.frame(cbind(y_hat,res))

ggplot(data = res_data, aes(x = y_hat, y = res)) + 
  geom_point()
...
res_data2 = as.data.frame(cbind(y_hat2,res2))

ggplot(data = res_data2, aes(x = y_hat2, y = res2)) + 
  geom_point()
...
ggplot(data = res_data, aes(x = y_hat, y = res)) + 
  ...
  geom_point(data = res_data2, aes(x = y_hat2, y = res2), color = "#E16462", fill = alpha("#E16462",0.4), pch=21, lwd = 1.5, size = 7)+
  geom_hline(aes(yintercept = 0), color="dark grey", lty = 2, lwd=1.3)+

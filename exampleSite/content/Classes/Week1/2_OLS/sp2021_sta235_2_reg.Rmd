...
summary(lm(log(Adj_Revenue)~bechdel_test, data=bechdel), digits = 4)$coeff
...
lm(log(Adj_Revenue) ~ bechdel_test + log(Adj_Budget) + Metascore + imdb + 
             USA + factor(Rated), data=bechdel)

```
```{r bechdel2r, echo=FALSE}
summary(lm(log(Adj_Revenue)~factor(pass_bechdel)+log(Adj_Budget) + Metascore + imdb, 
           data=bechdel, digits = 4))$coeff[1:5,]
...
- **.darkorange[Main idea:]** Transform everything to the same scale.
...
summary(lm(log_Adj_Revenue~bechdel_test+log_Adj_Budget + Metascore + imdb, 
           data=bechdel_std), digits = 4)$coeff[1:5,]

```
---
# Does this model explain well the observed data?

- Estimate the OLS model and predict $Y$ as following:
$$\widehat{\log{Revenue}} = \alpha + \hat{\beta}_1 Bechdel + \hat{\beta}_2 \log{Budget} + \hat{\beta}_3 Metascore + \hat{\beta}_4 IMDB$$
.box-4.medium.sp-after-half[$$Res = Y - \hat{Y}$$]

```{r bechdel4r}
summary(lm(log(Adj_Revenue) ~ bechdel_test +log(Adj_Budget) + Metascore + imdb, 
           data=bechdel))$adj.r.squared
...
```{r bechdel5, echo=T,results='hide'}

y_hat = predict(lm(log(Adj_Revenue) ~ bechdel_test+log(Adj_Budget) + Metascore + imdb, 
                   data = bechdel))

res = resid(lm(log(Adj_Revenue) ~ bechdel_test+log(Adj_Budget) + Metascore + imdb, 
               data = bechdel))

residual_data = as.data.frame(cbind(y_hat,res))

ggplot(data = residual_data, aes(x = y_hat, y = res)) + 
  geom_point()
...
ggplot(data = residual_data, aes(x = y_hat, y = res)) + 
  geom_point(color = "#E16462", fill = alpha("#E16462",0.4), pch=21, lwd = 1.5, size = 7)+
  ...
  xlab("Y_hat") + ylab("Residuals")+#ggtitle("How comfortable are you with R?")+
  ylim(-8,8)+
...
```{r bechdel6, echo=T,results='hide'}

y_hat2 = predict(lm(log(Adj_Revenue) ~ bechdel_test+log(Adj_Budget) + Metascore + imdb +
                     Year + Week + factor(Rated) + USA + English, 
                   data = bechdel))

res2 = resid(lm(log(Adj_Revenue) ~ bechdel_test+log(Adj_Budget) + Metascore + imdb +
                     Year + factor(Rated) + USA + English, 
                   data = bechdel))

residual_data2 = as.data.frame(cbind(y_hat2,res2))

ggplot(data = residual_data2, aes(x = y_hat2, y = res2)) + 
  geom_point()
...
ggplot(data = residual_data, aes(x = y_hat, y = res)) + 
  geom_point(color = "dark grey", fill = alpha("#E16462",0.4), pch=22, lwd = 1.5, size = 7)+
  geom_point(data = residual_data2, aes(x = y_hat2, y = res2), color = "#E16462", fill = alpha("#E16462",0.4), pch=21, lwd = 1.5, size = 7)+
  geom_hline(aes(yintercept = 0), color="dark grey", lty = 2, lwd=1.3)+

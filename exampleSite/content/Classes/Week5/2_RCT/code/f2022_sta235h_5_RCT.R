...
## First step is check for balance: This means, make sure baseline covariates are the same between both groups!
...
# The first argument provides the variables for the groups we want to compare (in this case treat=0 and treat=1)
...
# dinm_statistic provides the statistic that is returned to assess the difference between groups. By default it provides the standard error, but the p.value is easier to assess differences.


# Note that you can obtain the same values for the balance table using tidyverse code, but note that it doesn't teell us if the difference is statistically significant or not!:
...
# Task: Build binary variables for each level of education (e.g. mutate(educ_no_info = ifelse(education == 0, 1, 0)), and include those binary variables in your balance table. 
